...
import org.opensearch.spark.rdd.{Metadata => SparkMetadata}
...
private[spark] class ScalaMetadataExtractor extends PerEntityPoolingMetadataExtractor {
  ...
  override def getValue(metadata: InternalMetadata): AnyRef = {
    ...
    if (sparkEnum == null){
      ...
      entity match {
        case jmap: JMap[_, _] => jmap.asInstanceOf[JMap[Metadata, AnyRef]].get(sparkEnum)
        case smap: SMap[_, _] => smap.asInstanceOf[SMap[Metadata, AnyRef]].getOrElse(sparkEnum, null)
