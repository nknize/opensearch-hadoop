 ...
 * to OpenSearch for Spark Structured Streaming
 */
class OpenSearchCommitProtocol(@transient val commitLog: MetadataLog[Array[OpenSearchSinkStatus]]) extends Serializable {

  @transient lazy val logger: Log = LogFactory.getLog(classOf[OpenSearchCommitProtocol])
  ...
  def commitJob(jobState: JobState, taskCommits: Seq[TaskCommit]): Unit = {
    val commits = taskCommits.flatMap(_.statuses).toArray[OpenSearchSinkStatus]
  ...
  def commitTask(taskState: TaskState): TaskCommit = {
    if (recordsWritten > 0) {
      TaskCommit(Some(OpenSearchSinkStatus(taskState.taskId, executionStart, taskState.resource, recordsWritten)))
    } else {
      TaskCommit(None)
    }
  }
...
case class TaskCommit(statuses: Option[OpenSearchSinkStatus])