...
public abstract class InitializationUtils {
    ...
    public static void validateSettingsForWriting(Settings settings) {
        ...
        if (StringUtils.hasText(settings.getUpdateScriptFile())) {
            throw new OpenSearchHadoopIllegalArgumentException(
                    "Cannot use file scripts on ES 6.x and above. Please use " +
        ...
        if (StringUtils.hasText(settings.getMappingTimestamp())) {
            throw new OpenSearchHadoopIllegalArgumentException(
                    "Cannot use timestamps on index/update requests in ES 6.x " +
                            "and above. Please remove the [" + ConfigurationOptions.ES_MAPPING_TIMESTAMP
                            + "] setting.");
        }
        if (StringUtils.hasText(settings.getMappingTtl())) {
            throw new OpenSearchHadoopIllegalArgumentException(
                    "Cannot use TTL on index/update requests in ES 6.x and " +
